...
namespace PokerTests
{
    ...
    {
        [TestCase("2♠,3♠,5♠,4♠,A♠", "", "FiveCardDraw", "StraightFlush")]
        [TestCase("7♣,3♠,5♠,4♠,A♠", "", "FiveCardDraw", "HighCard")]
        ...
        {
            var sut = new PokerScorer(PokerVariant.Parse(variant));

            var result = sut.CalculateBestHand(PlayingCards.Parse(hand), PlayingCards.Parse(shared));
        ...
        public void CalculateBestHandWithCustomRankingsShouldReturnBestHand(string hand, string shared, string variant, string expectedBestHand)
        {
            ...
            var sut = new PokerScorer(PokerVariant.Parse(variant), rankings);
